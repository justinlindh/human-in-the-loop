...
- Your tools are in `docs/toolkit.md`: `npm run snap`, `blender/checks/scene.mjs`, `npm run lifecycle` and `npm run soak` for page errors, `scripts/sheet.sh` for before-and-after evidence, and `scripts/perf/bench.js --refs origin/main,<branch>` when a change could add DOM churn or per-frame work (its `mut/s`, `dom` and `cpu` columns, and `--profile`). Everything new works on touch as well as with a mouse.

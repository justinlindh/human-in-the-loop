...
- Your tools are in the toolkit (`npm run toolkit`, one file per tool in `docs/toolkit/`, with the prose in `docs/toolkit.md`): `npm run snap`, `blender/checks/scene.mjs`, `npm run lifecycle` and `npm run soak` for page errors, `node scripts/phone-check.js` for phone and tablet playability (run it on HUD, panel, camera-input and audio-unlock changes), `scripts/sheet.sh` for before-and-after evidence, and `scripts/perf/bench.js --refs origin/main,<branch>` when a change could add DOM churn or per-frame work (its `mut/s`, `dom` and `cpu` columns, and `--profile`). Everything new works on touch as well as with a mouse.

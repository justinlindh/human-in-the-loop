...
- Your tools are in the toolkit (`npm run toolkit`, one file per tool in `docs/toolkit/`, with the prose in `docs/toolkit.md`): `npm test` (the full suite whenever the sim changes), `npm run balance` with paired runs on the same seeds for any change that can move balance, `scripts/pace.js` for what a player sees and when, `scripts/perf/sim.js` for the cost of `tick()` across a long game, and `scripts/events/find.js` to find which seed, bot and week an event or decision happens in (the index rebuilds itself when the sim changes).

...
function atDesk(build, { x: lx = -0.5, z: lz = -0.28, rot = 0.3, y = TOP_Y, scale = DESK_PROP_SCALE, overhang = 0, group = false } = {}) {
  ...
  return (L, anchor, env) => {
    ...
    if (e) {
      ...
      // On the top: this desk, shrinking a little if need be. A prop without an anchor may then try the
      // nearest desks with room; one staged on the subject's desk stays with that desk, since its
      // moment picks the person by the desk the prop is on.
      ...
      if (onTop) {
        ...
        for (const d of near) {
          ...
          // One staged on the subject's desk must stay there for its moment, so it shrinks further.
          for (let k = 0; !spot && k < (mine ? 8 : 4); k++) { item.scale.multiplyScalar(0.88); spot = deskSpot(d, g, lx, lz, rot, overhang, around, others); }
...
const KEYS_HALF = 0.19;     // half a desk keyboard's width, with a little room
...
function deskGrid(e) {
  ...
  // The sitter's hands: in front of the keys. On a narrow desk (under 1.2 m across) only as wide as
  // the keyboard itself, so a desk that small still has free top.
  const hands = tx1 > tx0 && tx1 - tx0 < 1.2 ? KEYS_HALF : 0.3;
  mark(-hands, hands, -0.2, TOP_Z1);

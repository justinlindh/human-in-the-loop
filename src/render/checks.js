...
export async function runPropChecks(R, S, { dt = 1 / 30 } = {}) {
  ...
  // incident, a standup) and must not call anyone away: the moment still plays to the end.
  {
    ...
    results.push({ name: 'moment:printer', pass: done && worst < 0.01 && chin > 0, phases: [...phases], samples, insidePct: +(100 * worst).toFixed(2), worstWho, worstAt, chinGap: +chin.toFixed(3), chinWho });
    R.moments.full = false;
    step(30);
  }

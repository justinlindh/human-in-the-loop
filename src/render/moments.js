...
export function createMoments({ office, recs, walkTo, emote, getProps, note = () => {}, fx = null, parent = null, getYaw = () => Math.PI / 4, getCamera = null, momentCam = null, spotlights = null, isBusy = () => false, low = () => false }) {
  ...
  function wallSpot(from) {
    ...
    const nav = office.nav(), distances = new Map();
    function* candidates() {
      for (const [nx, nz] of walls.sort((a, b) => (a[0] * cam[0] + a[1] * cam[1]) - (b[0] * cam[0] + b[1] * cam[1]))) {
        ...
        search: for (let d = 0; d < 2 * half; d += 0.35) for (const s of [1, -1]) {
          ...
          const q = { x: along ? u : fixed, z: along ? fixed : u, yaw: Math.atan2(nx, nz), n: [nx, nz] };
          yield q;
          if (distances.has(q)) break search;
        }
      }
    }
    return choose(from, 'hammer', 'wall', {
      candidates: candidates(), needs: ['clear', 'reachable'],
      checks: { reachable(q) {
        const route = nav.path(from, q);
        if (route.length < 2) return false;
        distances.set(q, route.slice(1).reduce((sum, p, i) => sum + Math.hypot(p.x - route[i].x, p.z - route[i].z), 0));
        return true;
      } },
      // Both far walls can read clearly; use the one reached by the shorter clear route.
      score: (q) => distances.get(q),
    });
  }

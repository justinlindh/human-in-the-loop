...
import { modelCostPerCustomer, automationWeeklyCost } from '../../sim/economy.js';
  ...
  const view = liveView(
    ...
    (s) => {
      ...
      for (const m of MODELS) {
        ...
        const cost = modelCostPerCustomer(s, m.id);
        // One function at 100% on this model, priced by the sim's own formula.
        const autoFull = automationWeeklyCost({ ...s, automation: { ...s.automation, probe: { level: 1, model: m.id } } }, 'probe');
        ...
        const card = h('div.vendor', { style: { '--mc': m.color } },
          ...
          h('div.vbody', null,
            ...
            h('div.row.wrap.vcost', null,
              ...
              h('span.pill', { title: 'Weekly cost of one automation function at 100%', }, icon('agentic'), ` ${fmtMoney(autoFull)}/wk`)),
            h('div.row.wrap', null,

...
export function createReport(name) {
  ...
  return {
    ...
    finish({ out } = {}) {
      ...
      const line = (vals, r) => `${name.toUpperCase()} ${r ? tag(r) : '    '} ${vals.map((v, i) => v.padEnd(w[i])).join('  ')}${r?.known && (!r.pass || fixed(r)) ? `  (#${r.known}${r.pass ? ': passes now, drop the marker' : ''})` : ''}${r?.closed && !r.pass ? `  (#${r.closed} is closed but this still fails: fix it or reopen the issue)` : ''}${r?.closed && r.pass ? `  (#${r.closed} is closed: drop the marker)` : ''}`;

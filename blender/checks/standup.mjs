...
for (const c of CASES) {
  const { page, errors } = await H.openScene(c.seed ? `quality=low&seed=${c.seed}` : `quality=low&mock=${c.mock}`, { width: 640, height: 400 });
  ...
  console.log(`STANDUP ${res.pass && !errors.length ? 'ok  ' : 'FAIL'} ${c.seed ? `seed${c.seed}-w${c.weeks}` : c.mock}:${c.strip} ${JSON.stringify(res)}${errors.length ? ' errors: ' + errors[0] : ''}`);
  await page.close();
}
await H.close();

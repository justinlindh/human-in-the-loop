...
for sx in (-1, 1):
    ...
    # Temple arm: from the lens hinge back along the side of the head to the ear, as short
    # segments that hug the head surface instead of sticking out past it.
    rx, ry = HEAD_R * HEAD_S[0], HEAD_R * HEAD_S[1]
    kz = math.sqrt(max(0.0, 1 - (EZ / (HEAD_S[2] * HEAD_R)) ** 2))
    pts = []
    for a in (0.56, 0.9, 1.25, 1.6):                    # angle from straight ahead toward the ear
        pts.append((sx * (rx * kz + 0.008) * math.sin(a), -(ry * kz + 0.008) * math.cos(a), EZ + 0.004))
    pts.insert(0, (sx * (EX + 0.044), surf_y(EX + 0.044, EZ, 0.012), EZ))
    for k in range(len(pts) - 1):
        (x0, y0, z0), (x1, y1, z1) = pts[k], pts[k + 1]
        L = math.hypot(x1 - x0, y1 - y0)
        g.append(box(f'glarm{sx}{k}', (0.009, L + 0.006, 0.011), ((x0 + x1) / 2, (y0 + y1) / 2, (z0 + z1) / 2), 'plastic_charcoal', bevel=0,

...
- **Team tools:** every script, check and helper the agents use is documented in [`docs/toolkit.md`](docs/toolkit.md), which opens with a "When you need to..." index from a task to the tool for it, with one page per tool under [`docs/toolkit/`](docs/toolkit/). `npm run toolkit` prints them all.

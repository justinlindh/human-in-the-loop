...
#   - gh pr create/comment/review/edit text (title, body, heredoc bodies, body files) that contains
#     a local path (/home/..., /tmp/...).
# It looks only at commands mentioning pkill, pgrep, push or gh pr, and fails open on its own errors.
# Only deny() exits 2; any other failure exits otherwise, which Claude Code treats as allow.
set -f
input="$(cat)" || exit 0
command -v jq >/dev/null 2>&1 || exit 0
cmd="$(jq -r '.tool_input.command // empty' <<<"$input" 2>/dev/null)" || exit 0
case "$cmd" in *pkill*|*pgrep*|*push*|*"gh pr"*) ;; *) exit 0 ;; esac
...
if grep -qE 'gh[[:space:]]+pr[[:space:]]+(create|comment|review|edit)' <<<"$cmd"; then
  local_path='(/home/|/tmp/)'
  ...
  done < <(grep -oE -- "(--body-file|-F)[= ]+(\"[^\"]*\"|'[^']*'|[^[:space:];&|]+)" <<<"$cmd" || true)
fi
exit 0

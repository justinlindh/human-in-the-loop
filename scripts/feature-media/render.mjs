...
const REC = '1920x1080', FPS0 = 30;
// ffmpeg crop + scale + centre-cut to WxH, for a recording of W0xH0.
const vf = (o, [W0, H0]) => {
  ...
  for (const o of it.out ?? []) {
    ...
    try {
      if (o.path.endsWith('.webp')) {
        ...
        run('ffmpeg', ['-v', 'error', '-y', '-i', png, '-vf', vf(o, dim), '-quality', String(o.quality ?? 82), dest], o.path);

...
- **Pets**: a dog makes rounds leaving hearts, naps in a sunny spot or on a couch and chases the cat; a cat sleeps on a rack or a desk, knocks a pen off a desk and only visits its favourite person. Names include Kernel, Waffles, Null and Sudo. `capture 2-4-pets` `id: pet_request` `id: cat_request`
...
- **Moment camera**: during every registered spotlight, the camera glides in and back out, and lets go as soon as the player steers.
- **Spotlight**: `src/render/spotlight-kinds.js` is the shared list (`spotlight: true`): printer smash, user test, consultants, hammer swing, waffle party, music night, letter reading, fanning fumes, pizza gathering, pet carrier, screen recoil, company celebrations, and career path, Legend and top-level celebrations. Each holds the clock and queued notifications while the office animates, follows its action with the optional moment camera, and shows a caption with a tap-sized Skip. Steering releases the camera. At 2x scenes play at real-time speed; at 4x they skip and leave a caption toast. Opening a player menu ends the current scenes. Lingering props spotlight only their first staged animation, and a skipped visitor stays skipped until its prop goes away. Standups, coffee and pair games never hold the clock. Low keeps its lightweight prop emotes; growth celebrations omit the floor glow and confetti. See letters and fumes through `hearing_summons` and `coffee_machine_broke`; choose a career path, reach Legend or level 20 for the growth scenes.

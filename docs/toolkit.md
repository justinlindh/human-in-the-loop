...
| `npm run capture -- --group landing --out shots/landing/raw --no-webm` (prop stills with `--size 3840x2160`), then `scripts/reels/landing.sh shots/landing/raw shots/landing/site` | video | The landing page's stills and loops (humanintheloopgame-site). A bot plays a real game to the week before each moment and the game's own tick brings it live; the overlays are hidden. The script cuts each shot to the site's file names and sizes (`img/`, `img/loops/`, `media/loops/`), with a WebM and a poster per loop. The loops log the camera every frame; `node scripts/reels/camstats.mjs <capture dir> <id> [from] [to]` prints a cut's largest camera step and jerk. |

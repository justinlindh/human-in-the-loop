...
| `npm run capture -- --group nods --out shots/nods --size 1280x720 --fps 30 --audio --no-webm`, then `scripts/nods-reel.sh shots/nods shots/nods-reel.mp4` | art | The Office Space nods reel: seed 1 fast-forwarded to each nod's decision, the choice made, the staging framed; the script titles and joins the beats and lays the printer's music cue in at its moment start. |

...
|---|---|
| `blender/checks/clip.mjs [--rig] [--only=<pattern>[,<pattern>]]` | Characters against real furniture: seated poses in every mood, perk poses, and named prop moments (`moment:*`) sampled along their whole path, and pair games (`pairs:floor`, `pairs:garage`): the start rules allow one by a new table, and two people sent there get to play. `--only` runs just the cases whose names contain a pattern (`--only=printer` runs `moment:printer` in about a quarter of the time) and exits 1 when none matches; a narrowed pass is never recorded as a full pass, so the full run stays the gate. |
...
- `R.probe(<prop id or kind>)` gives a staged prop's `visible` and `occluder`, and `R.probeViews(id, [0, 1, 2, 3])` gives them for any actor or prop from each camera turn (n presses of E), for picking the view a clip or still should use.

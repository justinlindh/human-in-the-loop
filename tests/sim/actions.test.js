...
const PLAN_ORDER = [
  ...
  ['staff-upkeep', 85], ['unlocks', 86], ['goals', 87], ['chat', 88], ['prompts', 89], ['endgame', 90], ['history', 95],
];
...
describe('system order', () => {
  it('registered systems match the plan order table', () => {
    ...
    expect(got.map(([n]) => n)).toEqual(['calendar-start', 'ladder', 'standup', 'purpose', 'beats', 'moonshot', 'props', 'work', 'projects', 'products', 'marketing', 'vacation', 'strain', 'meaning', 'incentives', 'move-on', 'attrition', 'knowledge', 'market', 'for-sale', 'incidents', 'events', 'annual', 'economy', 'staff-upkeep', 'unlocks', 'goals', 'chat', 'prompts', 'endgame', 'history']);
  });
});

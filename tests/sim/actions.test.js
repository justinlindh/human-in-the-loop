...
const PLAN_ORDER = [
  ['calendar-start', 10], ['ladder', 11], ['standup', 12], ['work', 20], ['projects', 30], ['products', 40], ['marketing', 45], ['meaning', 50],
...
describe('system order', () => {
  it('registered systems match the plan order table', () => {
    ...
    expect(got.map(([n]) => n)).toEqual(['calendar-start', 'ladder', 'standup', 'work', 'projects', 'products', 'marketing', 'meaning', 'knowledge', 'market', 'incidents', 'events', 'annual', 'economy', 'staff-upkeep', 'unlocks', 'goals', 'chat', 'endgame', 'history']);
  });
});

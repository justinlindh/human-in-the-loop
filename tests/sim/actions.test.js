...
const PLAN_ORDER = [
  ['calendar-start', 10], ['ladder', 11], ['standup', 12], ['purpose', 13], ['work', 20], ['projects', 30], ['products', 40], ['marketing', 45], ['strain', 49], ['meaning', 50], ['incentives', 51],
...
describe('system order', () => {
  it('registered systems match the plan order table', () => {
    ...
    expect(got.map(([n]) => n)).toEqual(['calendar-start', 'ladder', 'standup', 'purpose', 'work', 'projects', 'products', 'marketing', 'strain', 'meaning', 'incentives', 'knowledge', 'market', 'incidents', 'events', 'annual', 'economy', 'staff-upkeep', 'unlocks', 'goals', 'chat', 'endgame', 'history']);
  });
});
